...
<div class="py-4 md:px-3">
  <h5 class="text-xl md:text-2xl mb-5 font-semibold text-gray-300">Edit Data Klub Sepak Bola</h5>
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
    ...
    <form action="/klub/{{ $klub->id }}" method="POST">
      ...
      <div class="block mb-5">
        ...
        <input type="text" id="nama_klub" name="nama_klub" class="block py-2 px-4 text-sm border rounded-lg w-full bg-gray-800/40 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500" placeholder="Masukkan nama klub" autocomplete="off" value="{{ $klub->nama_klub, old('nama_klub') }}">
        @error('nama_klub')
      ...
      <div class="block mb-5">
        ...
        <input type="text" id="kota" name="kota" class="block py-2 px-4 text-sm border rounded-lg w-full bg-gray-800/40 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500" placeholder="Masukkan kota" autocomplete="off" value="{{ $klub->kota, old('kota') }}">
        @error('kota')
          <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
        @enderror
      </div>
